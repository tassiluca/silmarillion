            ...
            <section class="comics <?php echo $section?>">
                ...
                <!--here insert other article to be scrolled-->
                    <?php foreach($templateParams[$section] as $product):?>
                        <article class="fade">
                            <div> <a <?php if(isset($templateParams["logged"])&& !$templateParams["logged"]){ echo 'class="disabled"';}?>href="article.php?id=<?php echo $product['ProductId']?>"><img src="img/comics/<?php echo $product["CoverImg"]?>" alt="copertina <?php echo $product["CoverImg"]?>"></a></div>
