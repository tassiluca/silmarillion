                ...
                <!--here insert other article to be scrolled-->
                    <?php foreach($templateParams[$section] as $product):?>
                        <article class="fade">
                            ...
                            <div><a href="index.php?action=wish&id=<?php echo $product['ProductId']?>"><img src="./img/favourite.svg" alt="Aggiungi ai preferiti"/></a><a href="index.php?action=toCart&id=<?php echo $product['ProductId']?>"><img src="./img/add.svg" alt="aggiungi al carrello"/></a><div><p><?php if(isset($product["DiscountedPrice"])){echo $product["DiscountedPrice"];}else{ echo $product["Price"];}?>€</p></div>
                            </div>
                        </article>
        <?php endforeach;?>
